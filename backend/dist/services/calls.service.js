...
const zendesk_service_1 = require("./zendesk.service");
...
class CallService {
    ...
    constructor() {
        ...
        this.zendeskService = new zendesk_service_1.ZendeskService();
        this.pdfParserService = new pdf_parser_service_1.PDFParserService();
    }
    ...
    transformCallData(callData) {
        ...
        const answeredCalls = callData.reduce((sum, item) => sum + item.answered, 0);
        const abandonedCalls = callData.reduce((sum, item) => sum + item.abandoned, 0);
        const presentedCalls = callData.reduce((sum, item) => sum + item.presented, 0);
        const answeredIn60Secs = callData.reduce((sum, item) => sum + item.answeredIn60Secs, 0);
        const bouncedCalls = callData.reduce((sum, item) => {
            console.log(`Adding ${item.bounced} bounced calls from ${item.date}, ${item.time}`);
    ...
    isNewerReport(fileName, existingItem) {
        ...
        const existingDateTime = this.parseDateTime(existingItem.date, existingItem.time);
        return fileDateTime > existingDateTime;
    }
    // Calculate calls per hour more accurately
    calculateCallsPerHour(callData) {
    ...
    generateResponseRateTrend(data) {
    ...
    getMaxTime(data, field) {
        if (data.length === 0)
            return "00:00:00";
        let maxSeconds = 0;
        data.forEach((item) => {
            const timeString = item[field];
    ...
    calculateAverageTime(data, field) {
        if (data.length === 0)
            return "00:00:00";
        let totalSeconds = 0;
        let count = 0;
