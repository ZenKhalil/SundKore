...
class PDFParserService {
    async parseCallCenterReport(pdfBuffer) {
    ...
    extractTableData(pdfData) {
        ...
        for (let pageIndex = 0; pageIndex < pdfData.Pages.length; pageIndex++) {
            ...
            for (const row of activityTable) {
                ...
                console.log(`Parsed bounce value from column 10: ${row[10]} => ${item.bounced}`);
                // Calculate percentage
                if (item.queued > 0) {
                    item.percentAnswered = Math.round((item.answered / item.queued) * 100);
                }
                items.push(item);
            }
